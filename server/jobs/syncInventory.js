...
/**
 * مزامنة المخزون لقاعدة مزامنة واحدة
 * @param {Object} rule - قاعدة المزامنة
 * @returns {Promise<Object>} - نتائج المزامنة
 */
const syncInventoryByRule = async (rule) => {
  ...
  try {
    ...
    const result = await syncService.syncInventory(sourceStore, targetStore, rule);
    ...
    logger.info(`اكتملت مزامنة المخزون من ${sourceStore.name} إلى ${targetStore.name} بنجاح`);
    return result;
  } catch (error) {
...
/**
 * مزامنة المخزون لجميع قواعد المزامنة النشطة
 * @returns {Promise<Array>} - نتائج المزامنة
 */
const syncAllInventory = async () => {
  try {
    logger.info('بدء مزامنة المخزون لجميع المتاجر');
    ...
    for (const store of stores) {
      // الحصول على قواعد المزامنة النشطة للمخزون
      const rules = await SyncRule.findActiveBySourceStore(store.id, 'inventory');

      for (const rule of rules) {
        try {
          // المزامنة بفترات قصيرة لتجنب تجاوز حدود API
          await sleep(1000);
          const ruleResult = await syncInventoryByRule(rule);
          results.push({
            ruleId: rule.id,
            ruleName: rule.name,
    ...
    logger.info('اكتملت عملية مزامنة المخزون لجميع المتاجر');
    return results;
  } catch (error) {
    logger.error('حدث خطأ أثناء مزامنة المخزون لجميع المتاجر:', error);
    throw error;
  }
};
